...
export default function LeadSenseUltimate() {
  ...
  const updateLead = async (id, payload) => {
    ...
    const res = await axios.patch(`http://localhost:5000/leads/${id}`, payload, {
      headers: {
  ...
  const addNote = async leadId => {
    ...
    const res = await axios.post(
      `http://localhost:5000/leads/${leadId}/notes`,
      { text },
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const runAI = async () => {
    ...
    try {
      const res = await axios.get("http://localhost:5000/ai-analysis", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const fetchRecommendations = async () => {
    ...
    try {
      const res = await axios.get("http://localhost:5000/ai-recommendations", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const explainLead = async leadId => {
    ...
    try {
      const res = await axios.get(`http://localhost:5000/lead-reasoning/${leadId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const generateOutreach = async (lead, channel) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/generate-outreach",
        { lead, channel },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const stats = useMemo(() => {
    ...
    return { total, avgScore, totalBudget };
  }, [filteredLeads]);
